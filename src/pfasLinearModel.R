...
covs = c(2,5,6,7,8,9,10,11,13)
combos = as.data.frame(t(combn(covs,2)))
repeats = cbind(c(6,7),c(9,10),c(11,13))
must_have = c(2, 5)
for (i in 1:nrow(combos)){
  for (j in 1:length(ncol(repeats))){
    if(repeats[j] %in% combos[i,] | sum(must_have %in% combos[i,]) < 2){
      combos[i,] = numeric(length = length(combos[i,]))
    }
  }
}
...
model {
  ...
  // data distribution
  for(i in 1:n){
    y[i] ~ lognormal(exp(mu[i]), sqrt(sigmasq));
  }
}
generated quantities {
  ...
  for (i in 1:n) log_lik[i] = lognormal_lpdf(y[i]|exp(mu[i]), sqrt(sigmasq));
}
"
...
get_fit = function(i_mod, all_mod_x){
  
  mod_x = all_mod_x[[i_mod]]
  print(ncol(mod_x))
  mod_data = list(n = length(total_pfas_df$CONCENTRATION_ngL),
                   ...
                   sigmasq0 = numeric(length = ncol(mod_x)) + 10^2,
